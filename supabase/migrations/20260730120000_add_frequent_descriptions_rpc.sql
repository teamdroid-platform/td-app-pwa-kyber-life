...
--
-- `type` and `status` are enums, so they are compared as text: `p_type` arrives
-- as a plain string and casting it the other way would raise on any label the
-- enum doesn't know.
...
RETURNS TABLE (description text, uses bigint)
LANGUAGE plpgsql
SECURITY DEFINER
